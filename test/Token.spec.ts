...
makeSuite('Token', () => {
  ...
  before('Setup', async () => {
    ...
    await execute('STRDY', { from: deployer }, 'setRoleCapability', TRANSFER_ROLE, '0xa9059cbb', true);
    await execute('STRDY', { from: deployer }, 'setRoleCapability', TRANSFER_ROLE, '0x23b872dd', true);
  ...
  it('Check BasicParameterSettings and Owner permission', async () => {
    ...
    expect(await read('STRDY', 'name')).to.be.eq('Sturdy');
    ...
    expect(await read('STRDY', 'balanceOf', deployer)).to.be.eq(ethers.utils.parseUnits('100000000', 18));

    await expect(
      execute('STRDY', { from: deployer }, 'mint', user1, 1)
    ).to.be.not.reverted;
    await expect(
      execute('STRDY', { from: deployer }, 'transfer', user2, 1)
    ).to.be.not.reverted;
  });
  ...
  it('Check Transfer', async () => {
    ...
    await expect(
      execute('STRDY', { from: user }, 'transfer', deployer, 1)
  ...
  it('Check Mint', async () => {
    ...
    await expect(
      execute('STRDY', { from: user }, 'mint', user, 1)
    ).to.be.revertedWith('UNAUTHORIZED');
